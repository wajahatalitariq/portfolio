...
/**
 * CyberLoader Component
 * 
 * An instant-loading cybernetic HUD preloader designed to display 
 * during server-side fetching and Three.js 3D asset downloading.
 */
const LOGS = [
...
export default function CyberLoader({ progressValue }: { progressValue?: number }) {
    const [mockProgress, setMockProgress] = useState(0);
    const [logIndex, setLogIndex] = useState(0);

    const progress = progressValue !== undefined ? progressValue : mockProgress;

    useEffect(() => {
        if (progressValue !== undefined) return;
